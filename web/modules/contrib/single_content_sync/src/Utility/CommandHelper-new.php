...
class CommandHelper implements CommandHelperInterface {
  ...
  public function getEntitiesToExport(string $entityType = 'node', bool $all_allowed_content = FALSE, string $entity_ids_to_export = NULL): array {
    ...
    if ($all_allowed_content) {
      $entities = $this->getAllAllowedEntities($allowed_entity_types);
    }
    elseif ($entity_ids_to_export) {
      $entities = $this->getSelectedEntities($entityType, $entity_ids_to_export);
    }
    else {
      $entities = $this->getEntitiesByTypes($entityType, $allowed_entity_types);
    }
    return $entities;
  }
   ...
   * {@inheritDoc}
   */
  public function getAllAllowedEntities($allowed_entity_types): array {
    $entities = [];
    foreach ($allowed_entity_types as $key => $bundle) {
      $entities_from_type = $this->entityTypeManager->getStorage($key)->loadMultiple();
      $entities = array_merge($entities, $this->filterEntitiesByBundle($entities_from_type, $bundle));
    }
  ...
  public function getEntitiesByTypes($entity_type, $allowed_entity_types): array {
    ...
    return $this->filterEntitiesByBundle($entities, $allowed_entity_types);
  }
  ...
  public function filterEntitiesByBundle($entities, $allowed_bundles): array {
    ...
    return array_filter($entities, function ($entity) use ($allowed_bundles) {
      return in_array($entity->bundle(), array_keys($allowed_bundles));
    });
  }
