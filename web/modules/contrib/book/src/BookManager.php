...
class BookManager implements BookManagerInterface {
   ...
   * {@inheritdoc}
   */
  public function getTableOfContents(int|string $bid, int $depth_limit, array $exclude = []) {
   ...
   * {@inheritdoc}
   */
  public function bookTreeAllData(int $bid, array $link = NULL, int $max_depth = NULL, int $min_depth = NULL) {
   ...
   * {@inheritdoc}
   */
  public function getActiveTrailIds(string $bid, array $link) {
   ...
   */
  protected function doBookTreeBuild(int $bid, array $parameters = []) {
   ...
   * {@inheritdoc}
   */
  public function bookTreeCollectNodeLinks(array &$tree, array &$node_links) {
   ...
   * {@inheritdoc}
   */
  public function loadBookLinks(array $nids, bool $translate = TRUE) {
   ...
   * {@inheritdoc}
   */
  public function bookTreeCheckAccess(array &$tree, array $node_links = []) {
    if ($node_links) {
   ...
   */
  protected function doBookTreeCheckAccess(array &$tree) {
    $new_tree = [];
    foreach ($tree as $key => $v) {
      $item = &$tree[$key]['link'];
   ...
   * {@inheritdoc}
   */
  public function bookLinkTranslate(array &$link) {
   ...
   */
  protected function buildBookOutlineData(array $links, array $parents = [], int $depth = 1) {
   ...
   *   Book tree.
   */
  protected function buildBookOutlineRecursive(array &$links, array $parents, int $depth) {
    $tree = [];
   ...
   * {@inheritdoc}
   */
  public function bookSubtreeData(array $link) {
